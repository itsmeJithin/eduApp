                    ...
                    <table class="table  dataTable no-footer" id="stripedTable" role="grid">
                        ...
                        <tr role="row">
                            ...
                            <th width="10%" class="sorting" tabindex="0" aria-controls="stripedTable">
                                {{ isFromExam ? 'Manage Exams' : 'Manage Questions' }}
                            </th>
                        </tr>
                        </thead>
                        <tbody>
                        ...
                        <tr role="row" class="odd" v-for="(subject,index) in subjects">
                            ...
                            <td class="v-align-middle" v-if="!isFromExam">
                                <button class="btn btn-outline-success" @click.prevent="manageQuestions(subject)">
                                    <i class="fa fa-plus mr-1"></i> Manage Questions
                                </button>
                            </td>
                            <td class="v-align-middle" v-else>
                                <button class="btn btn-outline-success" @click.prevent="manageExam(subject)">
                                    <i class="fa fa-gear mr-1"></i> Manage Exams
                                </button>
                            </td>
                        </tr>
...
export default {
    ...
    methods: {
        manageQuestions(subject) {
            this.$router.push({
                name: "manageQuestionPool",
                params: {classGroupSyllabusSubjectId: subject.class_group_syllabus_subject_id}
            })
        },
        manageExam(subject) {
            this.$router.push({
                name: "exams",
                params: {
                    classGroupSyllabusId: subject.pivot.class_group_syllabus_id,
                    classGroupSyllabusSubjectId: subject.class_group_syllabus_subject_id
                }
            })
        }
    }
