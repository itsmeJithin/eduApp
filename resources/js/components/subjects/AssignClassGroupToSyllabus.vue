<template>
    <div class="tab-pane slide-left padding-20 sm-no-padding active" id="tab11">
        <div class="row row-same-height">
            <div class="col-md-5 b-r b-dashed b-grey ">
                <div class="padding-30 sm-padding-5 sm-m-t-15 m-t-50">
                    <h2>Assign Class group to Syllabus!</h2>
                    <p>Here you can assign class group to syllabus and in the next step you can assign subjects to
                        this class groups.</p>
                    <p class="small hint-text">You can manage all subjects in this class groups in the next step. Do
                        this step carefully</p>
                </div>
            </div>
            <div class="col-md-7" id="class-assigning">
                <div class="padding-30 sm-padding-5">
                    <form role="form">
                        ...
                        <div class="form-group text-right mt-3">
                            ...
                            <button class="btn btn-primary" @click.prevent="assignClassGroupSyllabus">
                                <i class="fa fa-plus mr-1"></i> Assign Class Group
                            </button>
                        </div>
                    </form>
                </div>
        ...
        <div class="row">
            <div class="col-md-12">
                <ListClassGroupSyllabus
                    :refresh-data="refreshData"
                    @on-refresh-completed="changeRefreshStatus"/>
            </div>
        </div>
    </div>
</template>
...
import ListClassGroupSyllabus from "./ListClassGroupSyllabus";
import {required} from "vuelidate/lib/validators";

export default {
    name: "AssignClassGroupToSyllabus",
    components: {ListClassGroupSyllabus},
    data() {
        return {
            courses: [],
            selectedCourse: "",
    ...
    methods: {
        ...
        assignClassGroupSyllabus() {
            ...
            this.$axios.post(SubjectAPI.ASSIGN_CLASS_GROUP_SYLLABUS, data)
                .then(response => {
                    $('#class-assigning').unblock();
                    const responseData = response.data;
                    if (responseData.success) {
                        ...
                        $.showNotification("Class Groups assigned to subject successfully", "success");
                    } else {
                        $.showNotification(responseData.error, "error");
                    }
                })
                .catch(() => {
                    $('#class-assigning').unblock();
                    $.showNotification("Error occurred while assigning class groups to syllabus. Try again later", "error");
                })
        }
    }
}
